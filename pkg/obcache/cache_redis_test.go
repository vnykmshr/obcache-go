...
func TestCacheWithRedisStore(t *testing.T) {
	...
	config := NewRedisConfigWithClient(client).
		WithRedisKeyPrefix("test:cache:").
...
func TestRedisConfigBuilders(t *testing.T) {
	...
	if config2.StoreType != StoreTypeRedis {
		t.Fatal("Expected StoreType to be Redis after WithRedisAddr")
	}
